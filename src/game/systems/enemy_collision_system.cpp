...
static bool Filter(const Entity& entity) {
  return entity.Contains<CombatComponent>();
}

static void Enemy(Entity* entity_1, Entity* entity_2) {
  if (entity_1->GetId() == entity_2->GetId()) return;

  auto fc2 = entity_2->Get<CombatComponent>();
...
void EnemyCollisionSystem::OnUpdate() {
  for (auto& entity : GetEntityManager()) {
    if (entity.Contains<CombatComponent>()) {
      entity.Get<CombatComponent>()->Clear();
    }
  }
  ...
  for (auto& entity_1 : GetEntityManager()) {
    if (Filter(entity_1)) {
      for (auto& entity_2 : GetEntityManager()) {
        if (Filter(entity_2)) {
          Enemy(&entity_1, &entity_2);
        }
      }
    }
  }
}
EnemyCollisionSystem::EnemyCollisionSystem(EntityManager* entityManager, SystemManager* systemManager)
